...
#if defined(NDEBUG)
# error "Quasar cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Quasar Signed Message:\n";
using namespace std;
// Internal stuff
namespace {
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s QAC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(const CBlockIndex * pindexPrev , const Consensus::Params& consensusParams)
{
    ...
    }else if(pindexPrev->nHeight <BLOCK_TIME_100){ // block reward  subside
        nSubsidy = BLOCK_REWARD_COIN; //
    }else{
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Quasar: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
