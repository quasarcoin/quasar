...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Quasar database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Quasar datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Quasar datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Quasar 0.7 datadir."
    ;;
  3)
    echo "Detected Quasar pre-0.8 datadir."
    ;;
  4)
    echo "Detected Quasar 0.8 datadir."
    ;;
esac
